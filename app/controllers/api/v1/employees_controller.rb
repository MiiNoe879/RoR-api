  ...
  module V1
    class EmployeesController < ApplicationController
      ...
      def index
        ...
        render json: {data: tenant.employees.as_json(include: :mobility_entries, methods: :entity)}, status: :ok
      end
      ...
      def create
        ...
        ActiveRecord::Base.transaction do
          employee.mobility_entries = mobility_entries_params[:mobility_entries].map { |mobility|
            merged = mobility.merge({ employee_id: employee.id })
            MobilityEntry.new(merged)
          }
          tenant.save!
        end

        render json: {data: employee.as_json(include: :mobility_entries, methods: :entity) }, status: :created
      end
      ...
      def update
        ...
        if employee.present?
          ActiveRecord::Base.transaction do
            ...
            employee.mobility_entries.delete_all
            employee.mobility_entries = mobility_entries_params[:mobility_entries].map { |mobility|
              merged = mobility.merge({ employee_id: employee.id })
              MobilityEntry.new(merged)
            }
          ...
          render json: { data: employee.as_json(include: :mobility_entries, methods: :entity ) }, status: :ok
        else
      ...
      def employee_params
        params.permit(:firstName, :lastName, :email, :entity_id, :residence, :insider, :soc_sec, :internal_identification, :share_depository_account)
      end

      def mobility_entries_params
        params.permit(mobility_entries: [:from_date, :to_date, :override_entity_soc_sec, :entity_id])
      end
